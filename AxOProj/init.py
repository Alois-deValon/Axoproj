...
class Disk_model:
    ...
    '''
    def __init__(self, rin=1,rout=100, epsilon=0.2,alpha=1,Mstar=1,side='both',vphi_corr=False,gamma=0):
        """
        class constructor
        """
        if (rin < 0) or (rout < 0):
        ...
        if side not in ['both','top','bottom']:
            sys.exit('Error : Unknown side parameter, must be either "both", "bottom" or "top"')
        self.rin = rin
        self.rout = rout
    ...
    def plot_dynamics(self,plot_J=False,savename=""):
        ...
        cbar_label=[r'V$_R$ (kms$^{-1}$)',r'V$_Z$ (kms$^{-1}$)',r'V$_{\Phi}$ (kms$^{-1}$)']
        if plot_J:
            param[:,:,4]=param[:,:,4]*param[:,:,0]
            cbar_label[2]=r'RV$_{\Phi}$ (aukms$^{-1}$)'
        fig,ax=plt.subplots(nrows=1,ncols=3,sharey=True,sharex=True,figsize=(15,4))
        for i in range(3):
            im=ax[i].scatter(param[:,:,0],param[:,:,1],c=param[:,:,i+2],s=1,cmap=cmap[i])
        ...
        if savename:
            plt.savefig(savename+'.png',transparent=True, bbox_inches='tight', pad_inches=0,dpi=400)
        plt.show()
...
class Infall_model:
    ...
    def create_profile(self):
        ...
        costheta=-rd*np.cos(theta0)*np.power(np.sin(theta0),2)/R+np.cos(theta0)
        z=R*costheta
        theta=np.arccos(costheta)
        r=R*np.sin(theta)
        V_sph_r=(-30*np.sqrt(M)/np.sqrt(R))*np.sqrt(1+np.cos(theta)/np.cos(theta0))
        V_sph_theta=(30*np.sqrt(M)/np.sqrt(R))*(np.cos(theta0)-np.cos(theta))*np.sqrt( (np.cos(theta0)+np.cos(theta))/(np.cos(theta0)*np.power(np.sin(theta),2)))
        vphi=self.J_sign*(30*np.sqrt(M)/np.sqrt(R))*(np.sin(theta0)/(np.sin(theta)))*np.sqrt(1-np.cos(theta)/np.cos(theta0))
...
class BS_model:
    ...
    def create_profile(self):
        ...
        rbf=find_cubic_roots(1/(gamma*np.power(L0,3)),1/L0,-v0*zj/(L0*vj))
...
class Your_model:
    ...
    def create_profile(self):
        phi_1D=np.arange(0,360,self.dphi)*np.pi/(180.0)
        profile=np.dstack((self.r,self.z,self.vr,self.vz,self.vphi,self.i*self.r))
        return phi_1D,profile
...
if __name__=="__main__":
    ...
    """
    ###### DISK MODEL
    disk=disk_model(rin=1,rout=4,epsilon=1,side='both')
    disk.create_setup(step=0.1,dphi=1)
    disk.plot_dynamics()
    """

    """
    ###### WDS MODEL
    C=np.asarray([0.01,0.02,0.04])
    tau=np.asarray([850,500,220])
    eta=np.asarray([0.5,0.5,0.5])

    WDS=WDS_model(C=C,tau=tau,eta=eta)
    WDS.create_setup(step=1,zmax=3000,dphi=1)
    WDS.plot_dynamics()
    """
    ...
    DW=DW_model(r0=r0_geo,theta=theta,vp=vp,J=J)
    DW.create_setup(step=1,zmax=3000,dphi=1)
    DW.plot_dynamics(plot_J=True)
    ...
    test.plot_dynamics(plot_J=False,savename='perso')
    """
    print(hihi)
    ...
    DW=DW_model(r0=r0_geo,theta=theta,vp=vp,J=J)
    test=DW.create_profile(1,3000)


    plt.scatter(test[:,:,0],test[:,:,1],c=test[:,:,4]*test[:,:,0],s=1)
    plt.xlim(0,800)
    plt.ylim(0,3000)
    plt.show()
